...
/* gic_irq() comes from <linux/irqchip/mtk-gic-extend.h> in 4.9 */
 ...
 */
static void mt_gic_dist_save(unsigned int gic_nr)
{
	unsigned int gic_irqs;
 ...
 */
static void mt_gic_dist_restore(unsigned int gic_nr)
{
	unsigned int gic_irqs;
	unsigned int i;
	void __iomem *dist_base;
...
static void mt_gic_cpu_save(unsigned int gic_nr)
{
	int i;
	u32 *ptr;
	void __iomem *dist_base;
...
static void mt_gic_cpu_restore(unsigned int gic_nr)
{
	int i;
	u32 *ptr;
	void __iomem *dist_base;
...
#ifdef CONFIG_SMP
/* 4.9: CPU_STARTING notifiers are gone; use the cpuhp state machine, same
 * hook point the upstream 4.9 GIC driver uses. */
...
/* hwirq-flavoured helpers required by the 4.9 cirq driver; lifted
 * verbatim from this tree's misc/mediatek/irq/mt6735/irq.c (they use
 * the same GIC_DIST_BASE / INT_POL_CTL0 globals this driver defines). */
