...
/* g_IsNeedDoChipReset is owned by the wlan (gen2) driver, which is not
 * ported yet (Phase H part 2). Provide a weak fallback so /dev/wmtWifi
 * links standalone; the wlan driver's strong definition wins once it
 * is built in. */
UINT32 __weak g_IsNeedDoChipReset;
